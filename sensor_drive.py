...
import os
# 걷기 애니 확장은 Replicator(OmniGraph) import 전에 먼저 enable (그래프 충돌 회피)
if os.environ.get("WALK_ANIM", "1") == "1":
    import people_anim as _pa_early
    _pa_early.enable_extensions()
    for _ in range(10):
        app.update()
import json
import yaml
...
# 보행자 걷기 애니(omni.anim.people). 확장은 위에서 미리 enable(그래프 충돌 회피).
WALK_ANIM = os.environ.get("WALK_ANIM", "1") == "1"
CAM_W, CAM_H = 640, 360
