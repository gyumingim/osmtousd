...
def log(m): print(m, flush=True); open(OUT+"/run_mintest3.log", "a").write(str(m)+"\n")
open(OUT+"/run_mintest3.log", "w").close()
...
_c=UsdGeom.BBoxCache(Usd.TimeCode.Default(),[UsdGeom.Tokens.default_,UsdGeom.Tokens.render])
_tp=stage.GetDefaultPrim() or next(iter(stage.GetPseudoRoot().GetChildren()))
_r=_c.ComputeWorldBound(_tp).ComputeAlignedRange(); _mn,_mx=_r.GetMin(),_r.GetMax()
c1=(_mn[g1]+_mx[g1])/2; c2=(_mn[g2]+_mx[g2])/2; top=_mx[ui]; ground=_mn[ui]
gd=((_mx[g1]-_mn[g1])**2+(_mx[g2]-_mn[g2])**2)**0.5
...
for sq in range(12):
    ...
    Image.fromarray(rgb.get_data()[:, :, :3]).save(OUT+f"/mintest3_{sq}.png")
log("done")
app.close()
