...
## ⚠️ 무조건 규칙: 풀플로우 사전 공유 → 검토 → 실행
- **여러 단계로 이어지는 작업은 시작 전에 *전체 플로우(모든 단계)*를 사용자에게 전부 말하고, 검토·승인 받은 뒤에만 실행.** 한 단계씩 몰래 진행하다 일 키우는 것 금지.
- 풀플로우 설명에 반드시 포함: ①각 단계가 무엇을 하는지 ②무엇이 바뀌는지 ③**되돌릴 수 있는지** ④위험/실패 시 영향 ⑤필요한 사전조건(사용자가 줄 것 포함)
- **되돌리기 어렵거나 사용자/외부 시스템에 영향 주는 행동은 절대 임의로 실행 금지** — 사전에 위험까지 말하고 *명시적 승인* 받기. 예: 재부팅·전원·원격 머신 설정 변경·파일/데이터 삭제·소프트웨어 설치·네트워크/계정 변경·장시간 무인 작업·과금 발생.
  - (교훈 2026-06-26: 원격전용 5080을 풀플로우·위험 공유 없이 강제재부팅 → 사용자 접속까지 끊김. 이런 걸 막는 규칙.)
- **중간에 계획이 바뀌면 다시 전체를 말하고 승인** 받기. 승인은 그 단계 한정 — 다음 위험 행동에 자동 적용 안 됨.

## 역할 반복
- Planner 역할 -> 나한테 문제상황 받고, (해결방안도 있으면 받고), 어떻게 할지 설계, **풀플로우 설계후 나한테 승인받기**
